...
// Self-hosted AI provider for privacy-focused deployments
...
export class OllamaProvider implements AIProvider {
  ...
  private model: string = 'llama3.1:70b'; // Default model, configurable via env

  constructor() {
    this.baseURL = Deno.env.get('OLLAMA_BASE_URL') || 'http://localhost:11434';
    this.model = Deno.env.get('OLLAMA_MODEL') || this.model;
...
Respond with valid JSON only.`;
    ...
    try {
      ...
      const programData = JSON.parse(data.message.content);
...
Respond with valid JSON only.`;
    ...
    try {
      ...
      const data = await response.json();
      const adaptedWorkout = JSON.parse(data.message.content);
